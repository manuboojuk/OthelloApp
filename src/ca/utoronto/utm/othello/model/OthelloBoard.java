...
public class OthelloBoard {
	 ...
	 * 
	 * @param player P1 or P2
	 * @return an arraylist that contains all possible moves player can make
	 */
	public ArrayList<Move> getAvailableMoves(char player){
		ArrayList<Move> moves = new ArrayList<Move>();
		for(int row = 0; row < this.dim; row++) {
			for(int col = 0; col < this.dim; col++) {
				OthelloBoard othelloCopy = this.copy();
				if(othelloCopy.move(row, col, player))moves.add(new Move(row,col));
			}
		} 
		return moves;
	}
	 ...
	 * 
	 * @param player
	 * @return the number of token player has in the middle 2x2 area on the board
	 */
	public int presenceInMiddle(char player) {
		int count = 0;
		for (int i = 3; i <= 4; i++) {
			for (int j = 3; j <= 4; j++) {
